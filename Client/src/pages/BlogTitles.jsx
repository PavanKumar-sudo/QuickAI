...
const BlogTitles = () => {
  ...
  const blogCategories = [
     'General', 'Technology', 'Health', 'Travel', 'Finance', 'Lifestyle', 'Education', 'Food', 'Entertainment', 'Sports', 'Business', 'Science', 'Politics', 'Environment', 'Fashion'
    ]
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border'>
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-xl font-semibold'>AI Ttile Generator</h1>
        </div>
        <p className='mt-6 text-sm font-medium'>Keyword</p>
        <input onChange={(e) => setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='The Future of the Artificial intelligence is...' required/> 
